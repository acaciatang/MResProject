...
outname = 'test'
potentialTags = PythonTracking.findtags(img, outname)
...
for pts in potentialTags:
    ...
    if len(np.where(test > 100)[0])/(test.shape[0]*test.shape[1]) == 1.0 or len(np.where(test > 100)[0])/(test.shape[0]*test.shape[1]) < 0.5:
        #print(a)
        #print (len(np.where(test > 100)[0])/(test.shape[0]*test.shape[1]))
        a = a+1
        continue
    ...
    slopes = [(vertexes[i][1]-vertexes[i-1][1])/(vertexes[i][0]-vertexes[i-1][0])for i in range(4)]
    diff = [abs(slopes[i-2] - slopes[i])/((slopes[i-2] + slopes[i])/2) for i in range(2)]
    for d in range(2):
        if diff[d] > 0.1:
            lengths = [math.sqrt((vertexes[i][0]-vertexes[(i+1)%4][0])**2 + (vertexes[i][1]-vertexes[(i+1)%4][1])**2) for i in [d, d+2]]
            if lengths[0] > lengths[1]:
                short = d+2
            else:
                short = d
            change1 = copy.deepcopy(vertexes)
            change1[short][1] = change1[short-1][1] - change1[short-2][1] + change1[(short+1)%4][1]
            change1[short][0] = change1[short-1][0] - change1[short-2][0] + change1[(short+1)%4][0]
            change2 = copy.deepcopy(vertexes)
            s = short + 1
            change2[s][1] = change2[s-1][1] - change2[s-2][1] + change2[(s+1)%4][1]
            change2[s][0] = change2[s-1][0] - change2[s-2][0] + change2[(s+1)%4][0]
            
            average = []
            tags = []
            for v in [change1, change2]:
                edges = [math.sqrt((v[p-1][0]-v[p][0])**2 + (v[p-1][1]-v[p][1])**2) for p in range(len(v))]
                edge = math.floor(min(edges))

                OneCM = edge/0.3
                rows,cols, colour = cropped.shape
